 ...
 * @param num_constraints total number of constraints at each time step. Size
 * (`num_horizon,)`.
 * @param num_horizon horizon length (number of knot points, usually odd)
 *                    Valid inputs: [1,INT_MAX)
 * @param use_diagonal_costs Determines whether the quadratic cost matrices are diagonal or
 * dense. If true, the interface will assume all the parameters specifying the quadratic
 * cost terms `Q` and `R` are vectors of the diagonals, instead of dense matrices.
 * @param is_block_diagonal Determines if the cost function for the solver is block
 * diagonal, i.e. that the quadratic cross term `Hux` is zero.
 ...
 */
cocoa_Solver *cocoa_NewSolver(const int *num_states, const int *num_inputs,
                              const int *num_constraints, int num_horizon,
                              bool use_diagonal_costs, bool is_block_diagonal,
                              bool use_explicit_integration, cocoa_ERRORCODE *err);

cocoa_ERRORCODE cocoa_GetHorizonLength(const cocoa_Solver *solver, int *num_horizon);
cocoa_ERRORCODE cocoa_UsesDiagonalCosts(const cocoa_Solver *solver,
                                        bool *use_diagonal_costs);
cocoa_ERRORCODE cocoa_IsBlockDiagonal(const cocoa_Solver *solver, bool *is_block_diagonal);
cocoa_ERRORCODE cocoa_UsesExplicitIntegration(const cocoa_Solver *solver,
 ...
 * @param Q Quadratic state cost. Size is determined by whether the costs are dense or
 * diagonal.
 ...
 * @return error code
 */
cocoa_ERRORCODE cocoa_SetStateCost(cocoa_Solver *solver, const cocoa_float *Q,
                                   const cocoa_float *q, int k);

/**
 * @brief Set the quadratic and affine cost terms on the input for a single time step.
 *
 * @param solver An initialized solver
 * @param R Quadratic input cost. Size is determined by whether the costs are dense or
 * diagonal.
 ...
 * @return error code
 */
cocoa_ERRORCODE cocoa_SetInputCost(cocoa_Solver *solver, const cocoa_float *R,
                                   const cocoa_float *r, int k);
 ...
 *
 * Will return an error code if `cocoa_UsesExplicitIntegration` is true and `C` and `D` are
 * not NULL.
 *
 * @param solver An initialized solver
 * @param A State transition matrix. Size `(num_states[k+1], num_states[k])`.
 * @param B Input matrix. Size `(num_states[k+1], num_inputs[k])`.
 * @param C Next state matrix. Size (`num_states[k+1], num_states[k+1])`. If NULL, assume
 * identity.
 * @param D Next input matrix. Size (`num_states[k+1], num_inputs[k+1])`. If NULL, assume
 * zero.
 ...
 * @return error code
 */
cocoa_ERRORCODE cocoa_SetDynamics(cocoa_Solver *solver, const cocoa_float *A,
                                  const cocoa_float *B, const cocoa_float *C,
                                  const cocoa_float *D, const cocoa_float *f, cocoa_float h,
                                  int k);
...
// Options
/////////////////////////////////////////////
cocoa_ERRORCODE cocoa_SetOptionFloat(cocoa_Solver *solver, const char *option,
                                     cocoa_float value);
cocoa_ERRORCODE cocoa_SetOptionInt(cocoa_Solver *solver, const char *option, int value);

cocoa_ERRORCODE cocoa_GetOptionFloat(const cocoa_Solver *solver, const char *option,
                                     cocoa_float *value);
cocoa_ERRORCODE cocoa_GetOptionInt(const cocoa_Solver *solver, const char *option,
                                   int *value);
